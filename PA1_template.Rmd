...
```
The mean value is `r format(mean_total_steps, nsmall = 2)` and the median value is `r median_total_steps`.
...
```
The interval containing the max number of steps on average is `r max_steps_interval`.
...
```
The mean value is `r format(mean_total_steps_impute, nsmall = 2)` and the median value is `r format(median_total_steps_impute, nsmall = 2)`.
